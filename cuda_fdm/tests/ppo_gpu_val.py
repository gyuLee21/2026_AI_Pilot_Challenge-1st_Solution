...
def test_action_mapping():
    ...
    require_finite((ctrl, mid, grid), "action mapping")
    assert bool(ok and abs(mid[0]) < 1e-6 and abs(mid[3] - 0.5) < 1e-6 and grid_ok), "action mapping failed"
...
def test_gae_matches_reference():
    ...
    err = finite_max_abs_error(adv, ref, "GAE")
    print(f"(B) GAE vs 참조: 최대오차 {err:.3e} -> {'OK' if err < 1e-5 else 'FAIL'}")
    assert err < 1e-5, "GAE regression failed"
...
def test_train_integrity(iters=40, nenv=256):
    ...
    require_finite(tr.model.state_dict(), "trained parameters")
    ...
    assert not bad, "non-finite training regression"


def test_gating_and_weights():
    """EMA 게이팅과 기존 EMA-softmax PFSP 가중식을 확인한다."""
    torch.zeros(1, device="cuda")
    ...
    for _ in range(6):
        for e in tr.pool.active_entries():
            if not e["permanent"]:
                e["ema"] = 0.9
        tr.pool.gate_and_add(tr.model, tr.norm, cfg.selfplay_gate_threshold)
    ev = sum(1 for e in tr.pool.active_entries() if not e["permanent"])
    cap_ok = ev <= cfg.pool_evict_cap
    # 기존 PFSP는 우리 EMA 승률이 낮은(더 어려운) 상대를 우대한다.
    active = tr.pool.active_entries()
    active[0]["ema"] = 0.2
    active[1]["ema"] = 0.8
    w = tr.pool.weights(cfg.pool_sample_temp, cfg.pool_uniform_floor)
    w = w[[not e["retired"] for e in tr.pool.entries]]
    harder_bigger = float(w[0]) > float(w[1])
    emas = np.array([e["ema"] for e in active], dtype=np.float64)
    m = emas.size
    logits = -emas / cfg.pool_sample_temp
    logits -= logits.max()
    priority = np.exp(logits)
    priority /= priority.sum()
    p = cfg.pool_uniform_floor / m + (1.0 - cfg.pool_uniform_floor) * priority
    p /= p.sum()
    ref_err = finite_max_abs_error(w, p, "PFSP weights")
    print(f"(D) 게이팅: 6회 트리거 후 evictable {ev} (≤cap {cfg.pool_evict_cap})={cap_ok}")
    print(f"    EMA-softmax PFSP: 낮은 EMA 후보 우대={harder_bigger}, 참조식 오차 {ref_err:.2e}")
    print(f"    결과 -> {'OK' if cap_ok and harder_bigger and ref_err < 1e-5 else 'FAIL'}")
    assert cap_ok and harder_bigger and ref_err < 1e-5, "PFSP regression failed"
...
def test_exploiter():
    ...
    cfg = PPOGPUConfig(total_iterations=3, rollout_steps=12, update_epochs=2,
                       ...
                       milestone_period=3, exploiter_iters=4, exploiter_win_target=1.0)
    tr = PPOGPUTrainer(env, cfg)
    ...
    assert ok_batch and perm == 2 and tr.pool.capacity() == 4 + perm, "exploiter regression failed"
