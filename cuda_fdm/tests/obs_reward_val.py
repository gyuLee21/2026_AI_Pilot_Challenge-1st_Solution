...
from cuda_fdm.finite_checks import finite_max_abs_error, require_finite
...
def test_geometry():
    ...
    for i in range(M):
        e_dist = max(e_dist, finite_max_abs_error(g_dist[i], geo._get_distance(own[i], tgt[i]), "distance"))
        e_ata = max(e_ata, finite_max_abs_error(g_ata[i], geo._get_antenna_train_angle(own[i], tgt[i], False), "ATA"))
        e_aa = max(e_aa, finite_max_abs_error(g_aa[i], geo._get_aspect_angle(own[i], tgt[i], False), "aspect"))
        az, el = geo._get_los_angle(own[i], tgt[i])
        e_az = max(e_az, finite_max_abs_error(g_az[i], az, "azimuth"))
        e_el = max(e_el, finite_max_abs_error(g_el[i], el, "elevation"))
...
def test_obs_and_reward_trajectory(nenv=8, K=40, seed=3):
    ...
    for step in range(K):
        ...
        env.step(acts_t)
        require_finite(env.sim.states, f"trajectory[{step}].physical_state")
        s9 = env.state9().reshape(nac, 9)
        require_finite(s9, f"trajectory[{step}].state9")
        s9_np = s9.cpu().numpy()
        ...
        for a in range(nac):
            ...
            max_obs_err = max(max_obs_err, finite_max_abs_error(gpu_obs[a], cpu_obs, f"observation[{step},{a}]"))
        ...
        max_rew_err = max(max_rew_err, finite_max_abs_error(gpu_rew[0], cpu_r, f"reward[{step}]"))
        require_finite({key: getattr(bor, key) for key in ("hp", "fuel", "t_sec", "pqr", "hp_loss")},
                       f"reconstruction[{step}]")
...
def test_terminal_reward():
    ...
    cfg = dict(MR.MY_REWARD_CONFIG)
    cfg["shaping_reward_scale"] = 0.0  # terminal settlement only
    gpu_r = bor.compute_reward(s9, term, cfg=cfg).cpu().numpy()
    # Full remaining HP x damage_scale, symmetric for a single altitude exit.
    exp = {0: -10.0, 1: 10.0, 2: 10.0, 3: -10.0}
    err = 0.0
    for a, v in exp.items():
        err = max(err, finite_max_abs_error(gpu_r[a], v, f"terminal[{a}]"))
    # ac4: own HP0 → loss_reward(0) + (own alt 5000 정상) → 0
    err = max(err, finite_max_abs_error(gpu_r[4], cfg["loss_reward"], "terminal[4]"))
    ok = err < 1e-9
...
def test_kernel_vs_torch(nenv=16, K=50, seed=11):
    ...
    sim.load_seed(seeds)
    require_finite(sim.states, "kernel_regression.initial_physical_state")
    nac = 2 * nenv
    ...
    # reset 상태의 G를 양쪽 tracker에 먼저 넣어 첫 interval 사다리꼴 의미까지 비교한다.
    ...
    initial_s9 = torch.cat([torch.stack([n, e, d], 1), euler * R2D, vUVW * FT2M], 1)
    bor_t.initialize_reward_state(initial_s9, cfg=cfg)
    bor_k.kernel_init_reward_state(sim.states)
    for step in range(K):
        acts = np.zeros((nac, 4))
        ...
        require_finite(sim.states, f"kernel_regression[{step}].physical_state")
        # torch 경로 (state9 는 rl_env 계산 재사용)
        ecef_m, euler, vUVW, alt_ft = kinematics_state(sim.states)
        n, e, d = ned_from_ecef_altasl(ecef_m, alt_ft * FT2M)
        s9 = torch.cat([torch.stack([n, e, d], 1), euler * R2D, vUVW * FT2M], 1)
        require_finite(s9, f"kernel_regression[{step}].state9")
        ...
        max_obs = max(max_obs, finite_max_abs_error(obs_k, obs_t, f"kernel_obs[{step}]"))
        require_finite((rew_k, rew_t, term_k, trunc_k), f"kernel_reward[{step}]")
        ...
        if keep.any():
            max_rew = max(max_rew, finite_max_abs_error(rew_k[keep], rew_t[keep], f"kernel_reward[{step}]"))
        max_hp = max(max_hp, finite_max_abs_error(bor_k.hp, bor_t.hp, f"kernel_hp[{step}]"))
        max_pqr = max(max_pqr, finite_max_abs_error(bor_k.pqr, bor_t.pqr, f"kernel_pqr[{step}]"))
