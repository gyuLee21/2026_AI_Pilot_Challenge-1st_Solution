...
class Kernel:
    def __init__(self, ptx: bytes, name: str, max_registers: int = 0):
        ...
        self.force_sync = False  # diagnostic reference path, not the training default
        self.launch_count = 0
        self.context_sync_count = 0
        _cu_check(_cu.cuInit(0), "cuInit")
        dev = C.c_int(0)
        _cu_check(_cu.cuDeviceGet(C.byref(dev), self.device), "cuDeviceGet")
        ctx = C.c_void_p()
    ...
    def launch(self, grid, block, args, *, tensors=None):
        """Launch on the current PyTorch stream with allocator-safe tensor use.

        Producers, this kernel and downstream Torch operations share one stream.
        Callers moving inputs between streams must establish the usual wait_stream
        dependency first. record_stream protects the lifetime, not data readiness.
        Raw-pointer legacy tests without ownership information remain synchronous.
        """
        if torch.cuda.current_device() != self.device:
            raise ValueError("kernel and current PyTorch CUDA device differ")
        stream = torch.cuda.current_stream(self.device)
        if tensors is not None:
            for tensor in tensors:
                if tensor.device != torch.device("cuda", self.device):
                    raise ValueError("kernel tensor must reside on the kernel's CUDA device")
        n = len(args)
        params = (C.c_void_p * n)()
        keep = []
        ...
        _cu_check(_cu.cuLaunchKernel(
            ...
            C.c_uint(0), C.c_void_p(stream.cuda_stream), params, None), "LaunchKernel")
        self.launch_count += 1
        if tensors is not None:
            for tensor in tensors:
                tensor.record_stream(stream)
        if self.force_sync or tensors is None:
