...
(1) 수평 N/E는 공식-origin pymap3d 참조, D는 direct JSBSim MSL과 일치.
...
def test_bridge_vs_pymap3d(env):
    """state9 N/E and direct-MSL D versus independent CPU references."""
    ...
    for a in range(env.nac):
        ...
        tangent = pm.geodetic2ned(math.degrees(loc["mGeodLat"]), math.degrees(loc["mLon"]),
                                  alt_asl, ORIGIN_LAT_DEG, ORIGIN_LON_DEG, 0.0)
        ref = (tangent[0], tangent[1], -alt_asl)
        err = max(abs(got[a][i] - ref[i]) for i in range(3))
        maxerr = max(maxerr, err)
    print(f"(1) official N/E + direct-MSL D bridge: 최대오차 {maxerr:.3e} m  "
