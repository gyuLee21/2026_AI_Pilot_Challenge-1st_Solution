...
> 이 사본은 현재 실행 중인 scratch 100K 캠페인의 고정 런타임이다.
> `three_nine`와 `headon`은 서로 다른 run root를 사용하며, vNext는
> stage 1 live adapter로 동작한다. rollout은 의도적으로 64에 고정되어
> 증가하지 않는다. 현재 실행 절차와 알려진 제한은
> [ACTIVE_LEAGUE_100K_VNEXT.md](ACTIVE_LEAGUE_100K_VNEXT.md)를 우선한다.

## 미래 위치 보조학습 도입 (2026-08-31)

새 `train_gpu` 실행은 학습 전용 미래 위치 residual 예측을 기본 사용한다.
Git `origin/main2`의 상대 0.5초·자신 1초, 현재 own body 좌표계, 100m 정규화,
actor/critic 각각 MSE 계수 0.1을 유지했다. 관측 184D·제어 10Hz·reward·gamma·
PFSP는 바꾸지 않았다. rollout/상대 추론에서는 보조 head를 계산하지 않고,
제출 bundle에서도 제거한다. 실험 비교용 `--no-aux-pred`가 별도로 있다.

ON은 `cuda_mlp_flat_finite_horizon_future_aux_v5`, OFF는 기존 v4 프로토콜이다.
서로 다른 프로토콜을 자동 resume하거나 같은 CSV에 섞지 않는다. 기존 보류된
architecture search를 재개한 것은 아니다. 수식·마스크·로그·호환성 설명은
[FUTURE_AUXILIARY.md](FUTURE_AUXILIARY.md)를 참고한다.

## 2026-08-31 학습 경로 변경

사용자 결정에 따라 학습 계열은 **MLP**로 확정했다. `train_gpu`는 MLP 전용이며
`--hidden 672,672,672`처럼 폭·깊이만 지정한다. GRU 학습 옵션과 공용 recurrent
packing/padding updater, rollout hidden-state 버퍼를 제거했다. 기존 GRU checkpoint의
변환·평가 기능은 보존했다. 아래의 예전 GRU 학습 설명은 역사적 기록이다.

현재 updater는 GPU transition 단위 shuffle을 사용한다. PPO 손실식·epoch limit·
minibatch 분할 수는 유지하지만, 과거 sequence 단위 shuffle과 optimizer 순서는
다르므로 `cuda_mlp_flat_finite_horizon_v4`로 구분한다. 기존 v2/v3/legacy 학습을
자동 resume하지 않는다. reward/obs/gamma/PFSP 변경은 없다.

기존 architecture search는 STOP/무결성 보류 중이며 재개하지 않았다.
실측·검증: `runs/architecture_search/experiment_v1/performance_fixes_v1/REPORT.md`.
...
- **★claude164r 관측 + final-safe 보상 GPU 벡터화** (`cuda_fdm/obs_reward.py` `BatchObsReward`):
  ...
  - **보상**: HP차분 damage + 상대 기하 `G=S_own-S_enemy` + 저고도 safety potential + terminal.
    `S=0.50B+0.20L_safe+0.20K+0.10F`: broad 기하, 1.5초 상한 safe lead/direct 전환,
    거리·접근율 control-zone, 대회 damage phase 우선순위를 결합한다. 기하 예산은 ±5이고
    교차 구간은 이전/현재 `G`의 사다리꼴 적분을 사용한다.
...
- **step() 전체 종료+autoreset**: MSL 고도<1000ft(304.8m) / HP<=0 / 비유한 → terminated, SIM_TIME>200s → truncated.
...
- **학습 규약**: 관측=claude164r(my_observation, 커널 정합) · 보상=final-safe `MY_REWARD_CONFIG` ·
  γ=0.997 · **행동공간=discrete**(4채널×`num_bins`=21 균등격자 linspace(-1,1,21), 채널별 독립 Categorical) ·
  actor/critic 비공유 `obs(184)→512→512→GRU(256)→512→head` · recurrent sequence length=32 ·
...
- **opponent pool gated self-play**: env 기체0=**main actor**(학습), 기체1=**opponent**(pool 샘플 frozen).
  **main 기체 전이만** 버퍼(T,nenv)에 담겨 opponent 데이터 자동 배제.
  ...
  - **기존 EMA-softmax PFSP**: `p_i=0.5/m+0.5·softmax(-EMA_i/0.3)`. 낮은 main 승률의
    어려운 상대를 우대하면서 50% uniform 비중으로 모든 상대의 표본을 유지한다. 상대는 episode
    경계에서만 다시 뽑으므로 진행 중인 episode의 opponent는 바뀌지 않는다.
...
- **검증**: FDM 회귀, CUDA↔Torch obs/reward 정합, GRU rollout/update, checkpoint resume 및
  CUDA checkpoint→recurrent bundle 변환을 각각 검사한다.
...
- **실행**: `python -m cuda_fdm.train_gpu --save runs/gpu.pt --log runs/gpu.csv`
...
```powershell
# 처음부터 학습
python -m cuda_fdm.train_gpu --save runs/gpu.pt --log runs/gpu.csv

# 이어서 학습
python -m cuda_fdm.train_gpu --resume runs/gpu.pt --save runs/gpu.pt --log runs/gpu.csv

# CUDA checkpoint를 제출용 bundle로 변환
python -m cuda_fdm.gpu_ckpt_to_bundle --ckpt runs/gpu.pt --output-dir artifacts/models/team01/basic
```
...
```powershell
# 처음부터 학습
python claude_code/train.py --output-name ppo --output-tag exploiter2

# 이어서 학습
python claude_code/train.py --output-name ppo --output-tag exploiter2 --resume-from claude_code/models/ppo/exploiter2/iter_3673.pt

# CPU snapshot을 제출용 bundle로 변환
python claude_code/snapshot_to_bundle.py --snapshot claude_code/models/ppo/exploiter2/iter_5297.pt --output-dir artifacts/models/team01/basic
```

### 다른 bundle 또는 BT 모델과 성능 비교

```powershell
# bundle 대 BT
python claude_code/power_test.py --ownship-backend rl --ownship-bundle-dir artifacts/models/team01/basic --target-backend bt --target-bt-dll Shin_BT_best.dll --games 100

# bundle 대 bundle
python claude_code/power_test.py --ownship-backend rl --ownship-bundle-dir artifacts/models/team01/basic --target-backend rl --target-bundle-dir artifacts/models/team01/basic2

# bundle+MPC(altguard) 대 다른 bundle
python claude_code/power_test.py --ownship-backend altguard --ownship-bundle-dir artifacts/models/team01/basic --target-backend rl --target-bundle-dir artifacts/models/team01/basic --games 100
```

`altguard`의 상대를 BT로 바꾸려면 첫 번째 예시처럼 `--target-backend bt --target-bt-dll <DLL>`을 사용한다.
