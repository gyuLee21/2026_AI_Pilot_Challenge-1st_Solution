...
관측 크기뿐 아니라 CUDA 전용 history/갱신 순서를 metadata에 명시한다. 제출 provider는
그 contract로 관측을 재구성해야 하며, network 변환만으로 제출 동등성이 보장되지는 않는다.
...
from claude_code.observation_contract import CUDA_OBSERVATION_CONTRACT
...
def main():
    ...
    cuda_sd = ckpt["model"]
    require_finite(cuda_sd, "export.model")
    if ckpt.get("norm") is not None:
        require_finite(ckpt["norm"], "export.normalization")
    cfg = ckpt.get("cfg", {}) or {}
    # Auxiliary heads teach the shared trunk but are never deployed. Strip only
    # the declared four tensors; unexpected/missing keys still fail strict load.
    cuda_sd = inference_state_dict(cuda_sd, enabled=bool(cfg.get("aux_pred", False)))
    if args.observation_module != "claude_code.my_observation":
        raise ValueError("CUDA bundle export supports only the actual 214D training observation contract")

    # 구조 파라미터: 새 CUDA 경로는 CPU/제출용 GRU와 state_dict 이름까지 동일하다.
    # 예전 MLP 체크포인트는 기존 key-remap 경로로 계속 변환한다.
    hidden = tuple(cfg.get("hidden", (256, 256)))
    activation = cfg.get("activation", "tanh")
    gru_size = int(cfg.get("gru_size", 0) or 0)
    recurrent = cfg.get("architecture", "gru" if "actor_gru.weight_ih_l0" in cuda_sd else "mlp") == "gru"
    if recurrent:
        gru_size = gru_size or int(cuda_sd["actor_gru.weight_hh_l0"].shape[1])
        num_bins = int(cfg.get("num_bins", cuda_sd["actor_head.weight"].shape[0] // 4))
        obs_dim = int(cuda_sd["actor_encoder.0.weight"].shape[1])
        act_dim = int(cuda_sd["actor_head.weight"].shape[0] // num_bins)
        remapped = cuda_sd
    elif "actor_logits.0.weight" in cuda_sd:
        gru_size = 0
        head = cuda_sd[f"actor_logits.{2 * len(hidden)}.weight"]
        num_bins = int(cfg.get("num_bins", head.shape[0] // 4))
        obs_dim = int(cuda_sd["actor_logits.0.weight"].shape[1])
        act_dim = int(head.shape[0] // num_bins)
        remapped = cuda_sd
    else:
        gru_size = 0
    ...
    model = make_actor_critic(obs_dim=obs_dim, act_dim=act_dim, num_bins=num_bins,
                              ...
                              encoder_depth=int(cfg.get("encoder_depth", 2)))
    model.load_state_dict(remapped, strict=True)   # strict → 키/shape 완전 일치 검증(불일치 시 예외)
    model.eval()
    # 2026-09-04: 184 -> 214 (own/tgt 선가속도 30dim 추가, claude_code/my_observation.py).
    if obs_dim != 214 or act_dim != 4:
        raise ValueError("CUDA submission contract requires 214 observations and 4 controls")
    ...
    save_bundle(
        ...
        extra_metadata={
            ...
            "observation_contract": CUDA_OBSERVATION_CONTRACT,
            "policy_hz": 10,
            "action_repeat": 6,
            "training_protocol": ckpt.get("training_protocol", "legacy_mid_episode_pool_v1"),
            "auxiliary_trained": bool(cfg.get("aux_pred", False)),
            "auxiliary_contract": ckpt.get("auxiliary_contract"),
        },
    )
    ...
    print(f"  - obs_size={obs_dim} act={act_dim} num_bins={num_bins} "
          f"hidden={hidden} gru={gru_size} act_fn={activation} "
          f"obs_norm={'yes' if obs_norm else 'no'}")
