...
# Ten nanoseconds covers FP64 accumulation error at the 200-second game limit,
# but is many orders of magnitude smaller than a simulation/control step.
# Mirrored in cuda_fdm/gen/obs_kernel.cu and checked by timeout_terminal_val.
TIME_LIMIT_TOLERANCE_SEC = 1.0e-8


def time_limit_reached(sim_time, max_engage_time):
    """Inclusive task horizon; also works on NumPy/Torch time arrays."""
    return sim_time >= max_engage_time - TIME_LIMIT_TOLERANCE_SEC
...
def evaluate_termination(
    ...
    elif time_limit_reached(ownship_state[StateIndex.SIM_TIME], max_engage_time):
        truncated = True
