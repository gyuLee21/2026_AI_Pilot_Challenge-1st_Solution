...
struct Predictor {
  explicit Predictor(const std::filesystem::path& root,int maximum)
      : max_candidates(std::max(1,maximum)), thread_limit(0) {
    ...
    // Zero/unset preserves the standalone competition default.  Ray training
    // sets a small value per process to prevent N workers from each spawning
    // hardware_concurrency() threads for every CEM batch.
    if (const char* value=std::getenv("MPC_NATIVE_THREADS")) {
      char* end=nullptr;
      const long parsed=std::strtol(value,&end,10);
      if (end!=value&&parsed>0) thread_limit=static_cast<int>(parsed);
    }
  }
  int max_candidates;
  int thread_limit;
...
int MPC_EvaluateBatch(void* handle,const MPCPublicState* initial_state,
  ...
  try {
    ...
    const int requested=predictor->thread_limit>0
        ? predictor->thread_limit : static_cast<int>(hardware);
    const int worker_count=std::min(candidate_count,std::max(1,requested));
    if (worker_count==1) {
